...
class EventStreamEmitter {
  ...
  EventStreamEmitter({bool sync = true})
      : _controller = StreamController<Event>.broadcast(sync: sync);
  ...
  Future<MessageType> once<MessageType>(String topic) =>
      on<MessageType>(topic).first;
  ...
  void emit<MessageType>(String topic, MessageType data) =>
      _controller.add(Event(topic, data));
